...
本文的主要贡献在于：建立了可解释的热过程机理模型；将四个问题统一到同一参数化模型框架中；对优化问题给出明确的可行域、活跃约束和数值求解流程；同时通过敏感性分析与边界复核讨论模型稳定性。基于附件实验数据，本文采用升温段与冷却段分段换热系数辨识综合换热参数，拟合决定系数 $R^2=0.990$；问题二求得最大允许过炉速度 $v_{\max}=73.28$ cm/min；问题三在全部制程界限约束下求得最小超温面积 $A^*=409.48$ $^\circ$C$\cdot$s；问题四在面积几乎不损失的条件下将峰值两侧对称性指标 $D$ 由 0.893 降至 0.749（下降 $16.2\%$），实现面积与对称性的良好折中。
...
实际回焊过程中，升温阶段与降温阶段的对流换热强度存在差异，故本文将综合换热系数按温度响应方向分段：
\begin{equation}
\alpha(T)=\begin{cases}
\alpha_1, & T_a(x)>T(t)\ \text{（升温段）},\\
\alpha_2, & T_a(x)<T(t)\ \text{（冷却段）},
\end{cases}
\label{eq:alpha_pw}
\end{equation}
其中 $\alpha_1$、$\alpha_2$ 分别刻画升温与降温阶段的换热特性，由附件数据标定（见下节）。该处理在不增加模型结构复杂度的情况下显著提升拟合精度。
...
附件给出了实验工况下的观测温度数据（传送带速度 $v=70$ cm/min，设定温度 $(175,195,235,255,25)$ $^\circ$C）。模型采用"电路板进入回焊炉"为时间原点（$T(0)=25$），而附件数据以焊接区域中心温度达到 $30$ $^\circ$C（传感器开始工作）为记录零点。设 $t^*$ 为模型时刻中温度首次达到 $30.03$ $^\circ$C 的时刻，则传感器零点 $\tau$ 与模型时刻满足 $t=t^*+\tau$。标定时将模型曲线按该关系对齐后再与实测比较。

对于候选参数 $\boldsymbol\alpha=(\alpha_1,\alpha_2)$，由式~\eqref{eq:ode}与~\eqref{eq:alpha_pw}获得模型温度 $T_{\rm model}(t_k;\boldsymbol\alpha)$，以观测值 $T_{\rm obs}(t_k)$ 为基准建立最小二乘目标：
\begin{equation}
\boldsymbol\alpha^*
=
\arg\min_{\alpha_1,\alpha_2>0}
\sum_{k=1}^{n}
\left[
T_{\rm model}(t_k;\alpha_1,\alpha_2)-T_{\rm obs}(t_k)
\right]^2.
\label{eq:alpha}
\end{equation}
...
经数值求解，标定结果为
\[
\alpha_1^*=0.0205\ {\rm s}^{-1},\qquad
\alpha_2^*=0.0078\ {\rm s}^{-1}.
\]
模型与实测的拟合质量达到 $R^2=0.9900$、$RMSE=5.27$ $^\circ$C、最大绝对误差 $10.23$ $^\circ$C，说明分段换热系数模型能够较好地复现实测炉温曲线（标定对比见图~\ref{fig:calibration}）。升温段换热系数约为冷却段的 $2.6$ 倍，反映升温阶段高温环境对工件的强化换热。

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q1_calibration.png}
\caption{综合换热参数标定：模型曲线与实测数据对比}
...
\midrule
<<<<<<< HEAD
温区3中点 & 111.25 & 85.58 & 116.8\\
温区6中点 & 217.75 & 167.50 & 192.5\\
温区7中点 & 253.25 & 194.81 & 226.7\\
温区8结束处 & 304.00 & 233.85 & 243.2\\
=======
温区3中点 & 111.25 & 85.58 & 141.5\\
温区6中点 & 217.75 & 167.50 & 173.2\\
温区7中点 & 253.25 & 194.81 & 191.6\\
温区8结束处 & 304.00 & 233.85 & 223.7\\
>>>>>>> 4c5b71d853876299269ea102b51fedf5ba1f8e99
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q1_curve.png}
\caption{问题一炉温曲线（$v=78$ cm/min）及四个指定位置温度}
\label{fig:q1curve}
\end{figure}
...
\midrule
<<<<<<< HEAD
最大速度/cm/min & 79.3 & [65,100]\\
峰值温度/${}^\circ$C & 240.0 & [240,250]\\
最大升温斜率/${}^\circ$C/s & 2.85 & [0,3]\\
最小降温斜率/${}^\circ$C/s & -2.72 & [-3,0]\\
150--190℃时间/s & 61.8 & [60,120]\\
217℃以上时间/s & 40.2 & [40,90]\\
=======
最大速度/cm/min & 73.28 & [65,100]\\
峰值温度/${}^\circ$C & 240.0 & [240,250]\\
最大升温斜率/${}^\circ$C/s & 2.616 & [0,3]\\
最小降温斜率/${}^\circ$C/s & $-1.640$ & $[-3,0]$\\
150--190℃时间/s & 106.9 & [60,120]\\
217℃以上时间/s & 64.1 & [40,90]\\
>>>>>>> 4c5b71d853876299269ea102b51fedf5ba1f8e99
\bottomrule
\end{tabular}
\end{table}

由表~\ref{tab:q2result}可见，$v_{\max}=73.28$ cm/min 处峰值温度恰好等于下限 $240$ $^\circ$C（裕量约为 $0$），而其余指标均处于允许范围中部，因此\textbf{峰值温度下限约束为关键（活跃）约束}。物理上，过炉速度继续增大将使焊接区域在回流区停留时间过短，峰值温度跌破 $240$ $^\circ$C，无法满足锡膏充分回流的最低温度要求。

边界复核验证：取 $v_{\max}+0.5=73.78$ cm/min 重新计算，峰值温度降至 $240$ $^\circ$C 以下，制程界限不再全部满足，确认 $v_{\max}$ 确为可行域右端点。$v_{\max}$ 工况下的炉温曲线见图~\ref{fig:q2curve}。

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q2_curve.png}
\caption{问题二 $v_{\max}$ 工况炉温曲线（$217$、$240$、$250$ $^\circ$C 水平线已标注）}
\label{fig:q2curve}
\end{figure}
...
\midrule
<<<<<<< HEAD
$T_{1\sim5}$/℃ & 176.2 & [165,185]\\
$T_6$/℃ & 196.5 & [185,205]\\
$T_7$/℃ & 238.1 & [225,245]\\
$T_{8\sim9}$/℃ & 251.3 & [245,265]\\
$v$/cm/min & 67.8 & [65,100]\\
\midrule
$A^*/({}^\circ{\rm C}\cdot{\rm s})$ & 232.4 & 越小越优\\
$T_{pk}$/℃ & 240.3 & [240,250]\\
$\Delta t_{>217}$/s & 41.2 & [40,90]\\
$\Delta t_{150\sim190}$/s & 62.5 & [60,120]\\
$S_r$/℃/s & 2.92 & [0,3]\\
$S_d$/℃/s & -2.85 & [-3,0]\\
=======
$T_{1\sim5}$/℃ & 174.0 & [165,185]\\
$T_6$/℃ & 194.4 & [185,205]\\
$T_7$/℃ & 230.6 & [225,245]\\
$T_{8\sim9}$/℃ & 264.9 & [245,265]\\
$v$/cm/min & 85.33 & [65,100]\\
\midrule
$A^*/({}^\circ{\rm C}\cdot{\rm s})$ & 409.5 & 越小越优\\
$T_{pk}$/℃ & 240.0 & [240,250]\\
$\Delta t_{>217}$/s & 48.5 & [40,90]\\
$\Delta t_{150\sim190}$/s & 81.8 & [60,120]\\
$S_r$/℃/s & 2.552 & [0,3]\\
$S_d$/℃/s & $-1.645$ & $[-3,0]$\\
>>>>>>> 4c5b71d853876299269ea102b51fedf5ba1f8e99
\bottomrule
...
由表~\ref{tab:q3result}可见，最优解处峰值温度恰为下限 $240.0$ $^\circ$C，且 $T_{8\sim9}=264.9$ $^\circ$C 逼近上界 $265$ $^\circ$C，即\textbf{峰值温度下限与回流区温度上界为活跃约束}。物理解释：为减小 $217$ $^\circ$C 以上的超温面积，优化倾向于以"更高回流温度、更快过炉速度、更短高温停留"的方式，将峰值热负荷压缩到恰好满足最低工艺要求（$T_{pk}\ge240$）的程度。$A^*=409.5$ $^\circ$C$\cdot$s 对应的最优炉温曲线见图~\ref{fig:q3curve}。

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q3_curve.png}
\caption{问题三最优炉温曲线及超温面积（阴影部分）}
\label{fig:q3curve}
\end{figure}
...
\midrule
<<<<<<< HEAD
$T_{1\sim5}$/℃ & 178.9 & 176.2 & +1.5\% \\
$T_6$/℃ & 198.2 & 196.5 & +0.9\% \\
$T_7$/℃ & 239.0 & 238.1 & +0.4\% \\
$T_{8\sim9}$/℃ & 253.7 & 251.3 & +1.0\% \\
$v$/cm/min & 69.1 & 67.8 & +1.9\% \\
\midrule
$A^*/({}^\circ{\rm C}\cdot{\rm s})$ & 242.7 & 232.4 & +4.4\% \\
$D^*/{}^\circ{\rm C}$ & 12.8 & 16.5 & -22.4\% \\
$T_{\rm pk}$/℃ & 240.2 & 240.3 & -0.04\% \\
$\Delta t_{>217}$/s & 42.3 & 41.2 & +2.7\% \\
=======
$T_{1\sim5}$/℃ & 170.2 & 174.0 & $-2.2\%$\\
$T_6$/℃ & 198.6 & 194.4 & $+2.2\%$\\
$T_7$/℃ & 233.7 & 230.6 & $+1.3\%$\\
$T_{8\sim9}$/℃ & 264.9 & 264.9 & $\approx0\%$\\
$v$/cm/min & 86.14 & 85.33 & $+0.9\%$\\
\midrule
$A^*/({}^\circ{\rm C}\cdot{\rm s})$ & 410.0 & 409.5 & $+0.1\%$\\
$D^*/{}^\circ{\rm C}$ & 0.749 & 0.893 & $-16.2\%$\\
$T_{pk}$/℃ & 240.0 & 240.0 & $+0.0\%$\\
$\Delta t_{>217}$/s & 48.3 & 48.5 & $-0.5\%$\\
>>>>>>> 4c5b71d853876299269ea102b51fedf5ba1f8e99
\bottomrule
\end{tabular}
\end{table}

由表~\ref{tab:q4result}可量化面积与对称性之间的权衡：
\begin{equation}
\frac{A_4-A_3}{A_3}\times100\%=+0.1\%,
\end{equation}
\begin{equation}
\frac{D_4-D_3}{D_3}\times100\%=-16.2\%.
\end{equation}

即问题四在超温面积几乎不损失（仅增加 $0.1\%$）的条件下，将峰值两侧曲线对称性指标 $D$ 由 $0.893$ 降至 $0.749$（改善 $16.2\%$），体现了"面积优先、对称性次之"的主从折中策略的有效性。最优曲线及其峰前/峰后归一化对比分别见图~\ref{fig:q4curve}和图~\ref{fig:q4sym}。

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q4_curve.png}
\caption{问题四最优炉温曲线（兼顾面积与对称性）}
\label{fig:q4curve}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/q4_symmetry.png}
\caption{问题四峰值两侧归一化曲线对比}
\label{fig:q4sym}
\end{figure}
...
\]
其中 $\eta$ 取 $-10\%,-5\%,0,5\%,10\%$。分别重新计算问题一峰值温度与问题二 $v_{\max}$，结果列于表~\ref{tab:sens_alpha}。

\begin{table}[H]
\centering
\caption{综合换热参数扰动下主要结果变化}
\label{tab:sens_alpha}
\begin{tabular}{cccc}
\toprule
扰动 $\eta$ & $v_{\max}$/cm/min & 问题一峰值温度/$^\circ$C & 问题二 $\Delta t_{>217}$/s\\
\midrule
$-10\%$ & 无可行解 & 234.6 & ---\\
$-5\%$ & 69.6 & 236.4 & 67.7\\
$0$ & 73.3 & 238.0 & 64.1\\
...
可见 $v_{\max}$ 对 $\alpha$ 具有一定敏感性：$\alpha$ 每增大 $5\%$，$v_{\max}$ 约增大 $3.6$ cm/min；当 $\alpha$ 减小 $10\%$ 时换热不足，问题二在允许速度范围内无可行解。这说明 $\alpha$ 辨识精度是模型的重要误差来源，也解释了为何需以较高精度（$R^2=0.99$）标定分段换热系数。
...
当 $E_q$ 足够小时，认为数值离散误差不会影响最优方案的主要结论。本文分别以 $\Delta t=1.0,0.5,0.25,0.125$ s 求解，结果列于表~\ref{tab:sens_dt}。

\begin{table}[H]
\centering
\caption{不同 RK4 步长下的标定参数与问题一指标}
\label{tab:sens_dt}
\begin{tabular}{cccc}
\toprule
$\Delta t$/s & $\alpha_1$/s$^{-1}$ & $\alpha_2$/s$^{-1}$ & 问题一峰值温度/$^\circ$C\\
\midrule
1.0 & 0.0205 & 0.00774 & 237.9\\
0.5 & 0.0204 & 0.00775 & 237.9\\
0.25 & 0.0205 & 0.00776 & 238.0\\
0.125 & 0.0205 & 0.00774 & 238.0\\
\bottomrule
\end{tabular}
\end{table}

可见步长由 $0.25$ s 加密至 $0.125$ s 时，峰值温度变化 $<0.02$ $^\circ$C，标定参数基本不变，数值离散误差已充分收敛，故取 $\Delta t=0.25$ s 作为正式计算步长。
...
def temperature_rhs(t, T, alpha, speed, zone_temp):
    # alpha = (alpha1, alpha2)：升温段(环境高于工件)用 alpha1，冷却段用 alpha2
    x = speed * t

    Ta = ambient_temperature(x, zone_temp)

    a = alpha[0] if Ta > T else alpha[1]
...
\begin{verbatim}
# 时间对齐：模型以"进入回焊炉"为原点(T(0)=25)，实测以传感器 30℃ 为记录零点。
# 对候选参数求模型达到 30.03℃ 的时刻 t_star，再按 t = t_star + (t_obs - 19)
# 对齐后比较残差。
def calibration_error(
    ...
    i30 = np.argmin(np.abs(T_model - 30.03))
    t_star = np.interp(30.03, T_model[:i30+1], t_model[:i30+1])

    t_aligned = t_star + (t_obs - 19.0)
    ...
    return T_fit - T_obs      # 残差向量（供 least_squares 使用）
...
def calibrate_alpha(
    ...
    # 两参数最小二乘：升温段 alpha1、冷却段 alpha2
    result = least_squares(
        lambda p: calibration_error(
            (p[0], p[1]),
            t_obs,
            T_obs,
            zone_temp,
        ...
        x0=[0.024, 0.008],      # 网格搜索后取全局最优作为初值
        bounds=([0.001, 0.001], [0.3, 0.3])
    )

    return result.x            # (alpha1, alpha2)
\end{verbatim}
...
变量或指标 & 计算结果 \\
\hline
综合换热参数 $\alpha$ & $0.0135\ \mathrm{s^{-1}}$ \\
最大允许速度 $v_{\max}$ & $79.3\ \mathrm{cm/min}$ \\
\multirow{4}{*}{问题一位置温度} & $116.8\ \mathrm{^\circ C}$（温区3中点）\\
 & $192.5\ \mathrm{^\circ C}$（温区6中点）\\
 & $226.7\ \mathrm{^\circ C}$（温区7中点）\\
 & $243.2\ \mathrm{^\circ C}$（温区8结束处）\\
问题三超温面积 $A$ & $232.4\ \mathrm{^\circ C\cdot s}$ \\
问题四对称性指标 $D$ & $12.8\ \mathrm{^\circ C}$ \\
\hline
=======
\begin{tabular}{ccc}
\toprule
变量或指标 & 计算结果 & 单位\\
\midrule
升温换热参数 $\alpha_1$ & 0.0205 & s$^{-1}$\\
冷却换热参数 $\alpha_2$ & 0.0078 & s$^{-1}$\\
最大允许速度 $v_{\max}$ & 73.28 & cm/min\\
问题一位置温度 & 141.5/173.2/191.6/223.7 & ${}^\circ$C\\
问题三超温面积 $A$ & 409.5 & ${}^\circ$C$\cdot$s\\
问题四对称性指标 $D$ & 0.749 & ${}^\circ$C\\
\bottomrule
>>>>>>> 4c5b71d853876299269ea102b51fedf5ba1f8e99
\end{tabular}
\end{table}
