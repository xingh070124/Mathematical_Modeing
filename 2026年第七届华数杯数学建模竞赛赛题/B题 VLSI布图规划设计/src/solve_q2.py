...
L1 终端感知初解池 → L2 并行元启发（B*-树+SA 主 / SP+AGA 对照）
→ L3 双向反馈定界（凸松弛下界 + 可行证书 + gap）→ L4 场景适配。

输出：
  * 三组芯片总 HPWL（上界证书）、gap、可行性；
  * 主算法 vs 对照算法 HPWL 对比表；
  * 多起点收敛曲线与模块摆放可视化（Terminal 标注 + 轮廓框）。
"""
...
def solve_dataset(ds, cfg, net):
    ...
    # ---- L2 主算法：B*-树 + SA 多起点并行（增量 HPWL + 自适应γ）----
    tasks = []
    for i, sol in enumerate(pool):
        ...
        tasks.append((
            ...
            dict(widths=widths, heights=heights, rot=tree.rot.tolist(),
                 ...
                 budget_s=cfg.t_bstar, seed=cfg.seed + i * 101 + 7,
                 init=sol)
        ))
    results = [r for r in run_parallel(tasks, cfg.nproc) if r is not None]

    # ---- L2 精英池 + 扰动重启（重排大片 + 翻转旋转）----
    elites = sorted(results, key=lambda r: (r["overflow"], r["hpwl"]))[:3]
    rng2 = np.random.default_rng(cfg.seed + 999)
    restart_tasks = []
    for j, el in enumerate(elites):
        o2 = el["rot"].copy() if isinstance(el["rot"], np.ndarray) else \
            np.asarray(el["rot"])
        flip = rng2.choice(n, max(1, n // 8), replace=False)
        o2 = o2.copy()
        o2[flip] ^= 1
        init_seed = dict(xs=el["xs"], ys=el["ys"], rw=el["rw"], rh=el["rh"],
                         W=el["W"], H=el["H"], hpwl=el["hpwl"],
                         overflow=el["overflow"])
        tree = build_seed_tree(widths, heights, list(range(n)), o2, Lhat)
    ...
    # ---- 取主算法最优可行解（优先可行，再比 HPWL）----
    # 候选 = L1 初解池（skyline 可行证书）+ L2 SA 轨迹 + 扰动重启
    ...
    # ---- L2b 终端引力合法化精调（PeF 后处理）：在最优可行解上进一步压低 HPWL ----
    if best["overflow"] == 0:
    ...
    return dict(
        ...
        results_hpwl=[(r["hpwl"], r["overflow"]) for r in results],
...
def main():
    ...
    print(f"{'数据集':<8}{'轮廓L':>7}{'总HPWL(上界)':>14}{'凸松弛下界':>14}"
          f"{'gap':>9}{'可行':>6}{'SP+GA HPWL':>14}")
    for r in results:
        b = r["best"]
        sp_hp = f"{r['sp']['hpwl']:,.0f}" if r["sp"] else "—"
        print(f"{r['dataset']:<8}{r['Lhat']:>7}{b['hpwl']:>14,.0f}"
              f"{r['hpwl_lb']:>14,.0f}{r['gap']['gap_pct']:>8.2f}%"
              f"{'是' if r['feasible'] else '否':>6}{sp_hp:>14}")

    # ---- 多起点收敛表 ----
    print("\n" + "=" * 100)
    print("表2  多起点 B*-树+SA 各轨迹 HPWL 分布")
    print("=" * 100)
    for r in results:
        vals = ", ".join(f"{h:,.0f}(越界{o})" for h, o in r["results_hpwl"])
